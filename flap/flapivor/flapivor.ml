...
let get_current_commit project =
  List.hd (lines_of (git project "log --format=\"%H\" -n 1"))

let get_current_subject project =
  List.hd (lines_of (git project "log --format=\"%s\" -n 1"))

let is_updated project =
  let current_commit = get_current_commit project in
  let current_subject = get_current_subject project in
  ...
  if must_ignore then (
     Printf.printf
       "%s: Ignore commit %s as requested.\n%!" project current_commit;
     update ();
     false
  ) else  (
    try
...
let evaluate project =
  ...
  if not (Sys.file_exists logfile) then (
    ...
    command (Printf.sprintf
               "./flapitest %s %s %s \"%s\" %s"
               project commit logfile subject testsuites)
