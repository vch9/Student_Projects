...
(** On se donne la grammaire suivante pour les arbres de syntaxe
    ...
    Notez que les symboles non terminaux de cette grammaire sont :
       expression, term, et factor

    Par exemple, la définition du non terminal "expression" se lit :

      Une expression peut être produite de deux façons.

         1. Si on rencontre un mot du langage des "term"s suivi du symbole
            terminal "PLUS" puis suivi d'un mot du langage des "expression"s.

         2. Si on rencontre un mot du langage des "term"s.
...
let parse : token list -> e = fun tokens ->
  ...
  (** L'analyseur syntaxique suit un algorithme récursif et
      descendant qui correspond à l'analyse LL(1) vu en
      L2.
...
(** Un interprète produit la valeur entière correspondant
